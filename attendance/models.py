...
class SubjectAttendanceRecord(models.Model):
    ...
    subject's class, separate from (and repeatable through the day, unlike)
    the once-a-day building check-in in AttendanceRecord above."""
    ...
    subject = models.ForeignKey(
        "core.Subject", on_delete=models.CASCADE, related_name="attendance_records"
    )
    date = models.DateField()
    confirmed_at = models.DateTimeField(auto_now_add=True)
    confirmed_via = models.CharField(
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["student", "subject", "date"], name="one_subject_confirmation_per_day"
            )
        ]

    def __str__(self):
        return f"{self.student} in {self.subject} on {self.date}"
...
class NotificationLog(models.Model):
    ...
    related_subject_attendance = models.ForeignKey(
